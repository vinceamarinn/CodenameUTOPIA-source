...
func create_character(char_info): ## Creates a character from the base template using the given information, and places them in the map.
	...
	var char_sprites = load("res://subScenes/spriteFrames/" + char_name + "Sprites.tres")
...
func load_player(player_char): ## Loads the player character during overworld sections.
	var new_player = load("res://subScenes/templates/Player.tscn").instantiate()
	char_group.add_child(new_player)
	
	var char_sprites = load("res://subScenes/spriteFrames/" + player_char + "PLAYERSprites.tres")
...
func load_area(area_name:String, state:String) -> void: ## Handles the loading & processing of areas and the area's data based on the given state.
