...
func _compile_pattern_string(string: String):
	...
	pattern.suffix = string.substr(prefix_end + 7)
	if pattern.prefix == "":
...
func _get_keys_in_binary_scn_or_res(file_path: String, temp_path: String) -> Array:
	...
	if resource == null:
		print("StringKeys Error: Could not load resource: " + file_path)
		return []
	
	var error = ResourceSaver.save(resource, temp_path)
	if error != OK:
		print("StringKeys Error: Could not save temporary file: " + temp_path)
		return []
...
func _get_keys_in_binary_visual_script(file_path: String) -> Array:
	...
	if script == null:
		print("StringKeys Error: Could not load visual script: " + file_path)
		return []
	...
	scene.pack(node)
	var error = ResourceSaver.save(scene, "user://sk_temp.tscn")
	if error != OK:
		print("StringKeys Error: Could not save temporary scene file")
		return []
	
	var keys:= _get_keys_in_text_file("user://sk_temp.tscn")
	DirAccess.remove_absolute("user://sk_temp.tscn")
	return keys
...
func _get_keys_in_text_file(file_path: String) -> Array:
	...
	if file == null:
		print("StringKeys Error: Could not open file: " + file_path)
		return []
